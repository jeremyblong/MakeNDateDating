...
import styles from './restrictedImagesStyles.js';
import { BASE_ASSET_URL } from "@env";
import _ from "lodash";

const ViewRestrictedImagesOnly = (props) => {
        ...
        if (typeof passedImages !== "undefined" && passedImages.length > 0) {
            ...
            const newPromise = new Promise((resolve, reject) => {
                for (let idx = 0; idx < cumulatedArr.length; idx++) {
                    ...
                    const obj = Object.values(specific)[0];
                    const linky = obj.link;
                    
                    console.log("linky", linky)
                    
                    if (!_.has(specific, "empty")) {
                        newImageArr.push({ id: idx, image: linky, selected: idx === 0 ? true : false });
                    }
...
export default ViewRestrictedImagesOnly;