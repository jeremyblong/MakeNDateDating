...
const PageFiveContentData = ({ updateMainState, temp, handleContinuation, saveAuthenticationDetails }) => {
	...
	const handleImageSelection = async () => {
		...
		if (Platform.OS === "ios") {
			request(PERMISSIONS.IOS.CAMERA).then(async (resultOutterRequest) => {
				...
				if (!_.has(result, "didCancel") && _.has(result.assets[0], "base64")) {
						...
						if (res.data.message === "Uploaded successfully!") {
							...
							Toast.show({
								...
								onHide: () => {
									...
									saveAuthenticationDetails({ ...temp, profilePic: file });
					...
					Toast.show({
						type: 'error',
						text1: `Cannot process this image - choose another.`,
						text2: `Uploading your photo failed, please try another photo as this image isnt supported..`,
						visibilityTime: 2375,
						position: "bottom"
					});
				}
    ...
    const handleTakeNewImage = () => {
		if (Platform.OS === "ios") {
			request(PERMISSIONS.IOS.CAMERA).then(async (resultOutterRequest) => {
			   ...
			   if (resultOutterRequest === "granted") {
...
export default connect(mapStateToProps, { saveAuthenticationDetails })(PageFiveContentData);