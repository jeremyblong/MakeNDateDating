...
class LiveStreaming1v1Mentorship extends Component {
  constructor(props) {
    super(props);
    ...
    componentDidMount () {
      if (this.state.subscriberIds.length > 0 && this.state.intervalID !== null) {
    ...
    handleStart = (sessionId, streamId) => {
        ...
        axios.post(`${BASE_URL}/start/live/stream/broadcast/send/notification/alert`, configuration).then((res) => {
            if (res.data.message === "Successfully started!") {
                ...
                // console.log("passedData.otherUserID", passedData.otherUserID);
            } else {
                console.log("api error", res.data);
            }
        }).catch((error) => {
  ...
  joinVideoCall = () => {
    return (
      <SafeAreaView style={styles.fullView}>
        ...
        />
        <Text style={styles.headerText}>You're about to start a video conference with this user - this will send a notification to them for your scheduled appointment.</Text>
        <View style={styles.hr} />
        <Text style={styles.subbedText}>This will notify the user, just give them some time while we notify/call them alerting them of your phone call so please be patient while they work on joining the meeting.</Text>
        <View style={styles.centered}>
...
export default connect(mapStateToProps, { })(withNavigation(LiveStreaming1v1Mentorship));