...
import styles from "./viewRequestStyles.js";
...
const ViewCompanionshipRequests = ({ authData }) => {
    ...
    useFocusEffect(
        useCallback(() => {
            axios.get(`${BASE_URL}/gather/mentorship/requests`, {
                params: {
                ...
                if (res.data.message === "Gathered list!") {
                    ...
                    const { pendingMentorshipInvites } = res.data;
    ...
    const { pendingMentorshipInvites } = state;

    const renderEmptyHelper = () => {
      return (
        <Fragment>
          <View style={styles.centered}>
            <Text style={styles.headerText}>No data could be found/located...</Text>
            <Text style={styles.subheaderText}>We couldn't retrieve any relevant data for you at this time - this likely means you have no mentorship requests.</Text>
            <Image source={require("../../../assets/images/custom/nodata.png")} style={styles.nodataImage} />
          </View>
        </Fragment>
      );
    }
            ...
            <Fragment>
                <View style={[styles.contain, {backgroundColor: colors.card, marginTop: 17.75 }]}>
                    ...
                    <Text
                        ...
                        >
                            Initiated {moment(item.postedOn).fromNow(false)}
                        </Text>
                    <ScrollView contentContainerStyle={{ height: 275, flex: 1 }} style={{ height: 275, flex: 1 }}>
                    ...
                    <Button full style={{marginTop: 10}} onPress={() => navigation.navigate("ViewIndividualTherapyMentorshipRequest", { item })}>
        ...
        <Fragment>
            ...
            <View style={styles.container}>
                <FlatList
                    ...
                    data={pendingMentorshipInvites}
                    style={{ flexGrow: 1, flex: 1 }}
                    keyExtractor={(item, index) => `index-${index}`}
                    ListHeaderComponent={null}
                    renderItem={renderItem}
                    ListEmptyComponent={renderEmptyHelper}
...
export default connect(mapStateToProps, {  })(ViewCompanionshipRequests);