...
router.post("/", async (req, response) => {
    ...
    if (matching !== null) {
        const messageee = { 
            ...
            collapse_key: uuidv4(),
            notification: {
                title: `You have a new SUPER match request!`,
        ...
        const newNotification = {
            ...
            link: "super-match-request"
        };
    
        collection.findOneAndUpdate({ uniqueId: otherUserID }, { $push: { notifications: newNotification, requestedMatches: authenticatedUser.uniqueId }, $inc: { rank: Math.abs(Number(15)) }}, { returnDocument: 'after' }, (err, doc) => {
            if (err) {
        ...
        response.json({
            message: "Error - could not locate the desired user - no changes were made."
        })
    }
});
