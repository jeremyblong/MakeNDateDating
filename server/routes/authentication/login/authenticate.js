...
router.post("/", (req, resppppp, next) => {
    ...
    passport.authenticate('users', async (err, user, passedReq) => {
        if (err) {
            resppppp.json({
            ...
            if (!user) {
                resppppp.json({
                    message: "NO user found, User could NOT be authenticated - make sure you're using a valid 'email' and 'password' combination."
                })
            } else {
                console.log("check...");
                ...
                // const profilePic = user.profilePictures[user.profilePictures.length - 1].link
                ...
                //         avatar: `${config.get("assetLink")}/${profilePic}`
                //     })
                // };
