...
const NewUserMentorshipRegister = require("../../../schemas/auth/registerCompanionMentorAccount.js");
const { encryptString } = require("../../../crypto.js");
const stripe = require('stripe')(config.get("stripeSecretKey"));

router.post("/", (req, resppppp) => {

    const { businessAccountTempData } = req.body;

    const { deviceInfo, phoneNumber, allocationOfTime, billingTimeAllocation, intentInterest, hoursPerWeekWorking, howHearAboutUs, methodExpertises, specificRequests, yearsOfExperience, licensingInformation, schoolAndResume, profilePic, lastName, firstName, username, gender, birthdateRaw, birthdate, timezone, email, enrolled, password } = businessAccountTempData;

	const accountType = "mentorship/companionship";
	...
	fetch(url, options).then((response) => response.json()).then((response) => {
		...
		authy.registerUser({
			countryCode: "US",
			email: email.toLowerCase().trim(),
			phone: phoneNumber.unmaskedPhoneNumber // change this for PRODUCTION... -> phoneNumber.unmaskedPhoneNumber
		}, async (regErr, regRes) => {
			if (regErr) {
				...
				await stripe.accounts.create({
                    ...
                    if (errrrrrror) {
                      ...
                      res.send({
						...
						NewUserMentorshipRegister.register(new NewUserMentorshipRegister({
							firstName: firstName.toLowerCase().trim(),
							lastName: lastName.toLowerCase().trim(),
							password: encryptString(password),
							coreMentorshipAccountInfo: {
								allocationOfTime, 
								billingTimeAllocation, 
								intentInterest, 
								hoursPerWeekWorking, 
								howHearAboutUs, 
								methodExpertises, 
								...
								schoolAndResume, 
								address: businessAccountTempData.address, 
								timezone
							},
							...
							accountType,
							gender, 
							phoneNumber: phoneNumber.unmaskedPhoneNumber,
							securityAuthenticationLogs: [deviceInfo],
							rank: 500,
							rankedArr: []
