...
router.post("/", async (req, resppppp) => {
    ...
    if (otherUser !== null && authedUser !== null) {
        const newNotification = {
            ...
            data: {
                title: `You have NOT been chosen for the in-app game/competition event (${authedUser.firstName} ~ @${authedUser.username}) you applied to...`,
                body: `You have been declined entry to the in-app game/competition or event you recently applied to - the listing user (${authedUser.firstName} ~ @${authedUser.username}) - sorry for the bad news...`
            },
            from: authedUser.uniqueId,
            link: "bachelor-verdict-denied"
        };
        ...
        bachelorCollection.findOneAndUpdate({ id: listingID }, { $pull: {
            joined: {
                uniqueId: from
            }
